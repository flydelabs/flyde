import { VisualPart, isRefPartInstance, isInlinePartInstance, isVisualPart, FlydeFlow, ResolvedFlydeFlow, isCodePart } from "@flyde/core";
...
const getRefPartIds = (part: VisualPart): string[] => {
  ...
  export function resolveDependencies( flow: FlydeFlow, mode: ResolveMode, fullFlowPath: string): ResolvedFlydeFlow['dependencies'] {
          ...
          .reduce((acc, path) => {
            ...
            if (/.(js|ts)x?$/.test(path)) {
                try {
                    ...
                    if (isCodePart(part)) {
                        return [...acc, {flow: {part, imports: {}}, path}];
                    } else {
                        if (typeof part === 'object') {
                            const parts = Object.values(part).filter(isCodePart).map(part => {
                                return {flow: {part, imports: {}}, path};
                            })
                            return [...acc, ...parts];
                        }
                    }
