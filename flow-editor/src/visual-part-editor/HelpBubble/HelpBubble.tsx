import { Classes, Dialog, H4, HotkeysDialog2, IHotkeysDialogProps, Menu, MenuDivider, MenuItem, setHotkeysDialogProps } from '@blueprintjs/core';
...
import { currentHotkeys, HotkeysMenuData } from '../../lib/react-utils/use-hotkeys';
...
function hotkeyToBpHotkey(hotkey: {key: string, menuData: HotkeysMenuData}): HotkeysDialog2Props['hotkeys'][0] {
	return {
		combo: hotkey.key,
...
export const HelpBubble: React.FC<HelpBubbleProps> = () => {
	...
	const bpHotkeys = Array.from(currentHotkeys.entries())
		.map(([keys, menuData]) => hotkeyToBpHotkey({key: keys, menuData}));
	...
	const hotkeysModal = <Dialog isOpen={hotkeysModalOpen} onClose={() => setHotkeysModalOpen(false)}>
			<div className={Classes.DIALOG_BODY}>
				{groupsArray.map(([group, hotkeys]) => (<React.Fragment>
						<H4>{group}</H4>
						{hotkeys.map(hotkey => {
	...
	const menu = (<Menu>
		...
		<MenuItem text='Documentation' href='https://www.flyde.dev/docs' target='_blank'/>
		<MenuDivider/>
		<MenuItem text='Discord' href='https://discord.gg/x7t4tjZQP8'  target='_blank'/>
	</Menu>)
	return <div className='help-bubble' data-tip="Help">
		<CustomReactTooltip/>
