...
export const InlineCodeModal: React.FC<InlineCodeModalProps> = React.memo(function ValueEditorModal(
  ...
  const [value, setValue] = React.useState(isDefined(initialValue) ? initialValue : defaultValue);
  ...
  const inputPinExplanation = (
    <div>
      <p>
        ...
        <br /> For example:
        <Code>{"`Hello {inputs.name}`"}</Code> will expose an input pin named "name"
      </p>
      <p>
        You can also use ternery expressions, arithmetic operators and more!
        <br />
        For example:
        <Code>{"inputs.a > inputs.b ? inputs.a + 42 : Math.random() * inputs.b"}</Code>
      </p>
    </div>
  );
  ...
  const typeExplanationValue = (
    <span>
      ...
      <Code>["bob", "alice", "dave"]</Code>.<br/> Dynamic inputs may also be used
    </span>
  );
  const typeExplanationInline = (
    <span>
      ...
      <br /> Example:
      <Code>const a = inputs.a; const b = inputs.b; return Math.floor(a/b);</Code>
    </span>
  );
    ...
    if (!hasReturn && type === CodePartTemplateTypeInline.FUNCTION) {
      return (
        <Callout intent={Intent.DANGER}>
          When using the "function" type you are expected to <Code>return</Code> a value.
          <br />
  ...
  React.useEffect(() => {
    if (value === defaultValue && type === CodePartTemplateTypeInline.FUNCTION) {
      setValue(`return ${defaultValue}`);
    }
    if (value === `return ${defaultValue}` && type === CodePartTemplateTypeInline.VALUE) {
    ...
    >
      <main className={classNames(Classes.DIALOG_BODY)} onKeyDown={onKeyDown} tabIndex={0}>
        ...
        <Callout intent={Intent.NONE}>
          Input pins detected: {vars.length ? vars.map((v) => <Code key={v}>{v}</Code>) : "None"}
