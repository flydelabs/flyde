...
export const LayoutDebuggerItem: React.FC<LayoutDebuggerItemProps> = (props) => {
...
export interface LayoutDebuggerProps {
  extraDebug: LayoutDebuggerItemProps[], vp: ViewPort, part: GroupedPart,
  mousePos: Pos
}
...
export const LayoutDebugger: React.FC<LayoutDebuggerProps> = (props) => {
  const {extraDebug, vp, part, mousePos} = props;


  if (!isDebug) {
    return null;
  }
  const itemElems = extraDebug.map((props, idx) => <LayoutDebuggerItem {...props} key={idx}/>);

  const basePartElems = [...part.instances.map(i => i.pos), ...values(part.inputsPosition), ...values(part.outputsPosition)]
    .map(pos => <LayoutDebuggerItem pos={pos} viewPort={vp} size={{width: 0, height: 0}} color='red'/>)


  const  mouseRenderedPos = logicalPosToRenderedPos(mousePos, vp);
  const viewPortData = <div className='viewport-data'>
    <div>Pos: <PosView pos={vp.pos}/></div>
    <div>Zoom: {vp.zoom.toFixed(2)} </div>
    <div>Mouse (logical): <PosView pos={mousePos}/></div>
    <div>Rendered (rendered): <PosView pos={mouseRenderedPos}/></div>
  </div>
