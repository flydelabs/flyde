...
import { InputPinConfig, InputPin, isStaticInputPinConfig, staticInputPinConfig, queueInputPinConfig, InputMode, stickyInputPinConfig, INPUT_MODES, InputPinMode } from "@flyde/core";
...
const InputModeSelect: React.FC<{config: InputPinConfig, onChangeConfig: (config: InputPinConfig) => void}> = (props) => {
...
export const InstancePanelInputPinView: React.FC<InstancePanelInputPinViewProps> = (props) => {
	...
	const _onToggleVisible = useCallback((e: any) => {
	  onToggleVisible(id, e.target.checked);
	}, [id, onToggleVisible]);
  
	const _onChangeConfig = useCallback((config: InputPinConfig) => {
	...
	const onChangeRawValue = useCallback((value: string | undefined) => {
	...
	const isDisabled = mode === 'static' ? false : pin.mode === 'required' || (pin.mode === 'required-if-connected' && props.connected && visible);
	...
	return (
	  <Card className="pin-view">
		<h5 className="bp3-heading">
		  ...
		  <Switch checked={visible} disabled={isDisabled} inline onChange={_onToggleVisible} innerLabel='Hidden' innerLabelChecked='Visible'/>
		</h5>
		<FormGroup
		  ...
		  // labelInfo="(required)"
	  >
		<InputModeSelect config={config || queueInputPinConfig()} onChangeConfig={_onChangeConfig}/>
	  </FormGroup>
