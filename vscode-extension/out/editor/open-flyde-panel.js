...
const getViteAssets = async (root, webview, isDev) => {
    ...
    if (indexEntry.file) {
        const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(root, "webview-dist", indexEntry.file));
        scriptTags += `<script type="module" src="${scriptUri}"></script>`;
    }
    // Add CSS files
    if (indexEntry.css) {
        for (const cssFile of indexEntry.css) {
            const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(root, "webview-dist", cssFile));
            styleTags += `<link href="${styleUri}" rel="stylesheet">`;
        }
    }
    return { scriptTags, styleTags };
};
async function getWebviewContent(params) {
    const { extensionUri, relativeFile, port, webview, initialNode, executionId, darkMode, } = params;
...
//# sourceMappingURL=open-flyde-panel.js.map