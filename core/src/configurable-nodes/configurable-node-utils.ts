...
import { isConfigurableValue } from "../node/configurable-value";

import { nodeInput } from "../node";

// Import the InputConfig type from configurable-nodes
import { InputConfig } from "./configurable-nodes";
...
export function generateConfigEditor<Config>(
  ...
  const fields = Object.keys(config).map((key) => {
    ...
    let fieldType: ConfigurableFieldDefinition["type"];
  ...
  return {
    type: "structured",
    fields: fields as ConfigurableFieldDefinition[],
  };
}
...
export function evaluateFieldVisibility(
  ...
  allFields: Record<string, ConfigurableFieldDefinition>,
  config: Record<string, any>
): boolean {
