...
export * from "./configurable-node-utils";

import {
  isConfigurableValue,
  configurableValue,
  SecretTypeData,
} from "../node/configurable-value";
...
export function processConfigurableNode(node: CodeNode, secrets: Record<string, string> = {}): InternalMacroNode<any> {
  ...
  if (isAdvanced) {
    ...
    const allFieldDefinitions = Object.keys(node.inputs).map((key) => {
      ...
      } as ConfigurableFieldDefinition;
    });
    ...
    const fieldDefinitionsMap = allFieldDefinitions.reduce((acc, field) => {
      ...
      return acc;
    }, {} as Record<string, ConfigurableFieldDefinition>);

    // Function to recursively build group fields
    function buildGroupFields(groupKey: string): ConfigurableFieldDefinition[] {
      ...
      runFnBuilder: (config) => {
        return (inputs, outputs, adv) => {
          const inputValues = Object.keys(node.inputs).reduce((acc, key) => {
            const configValue = config[key] ?? configurableValue("dynamic", `{{${key}}}`);
